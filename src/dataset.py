...
def build_audio_resolver(
    ...
    def resolver(r: str) -> str:
        ...
        if os.path.isabs(r) and os.path.exists(r):
            return r
        rr = r.lstrip("./")
        return os.path.join(best_base, rr) if best_mode == "rel" else os.path.join(best_base, os.path.basename(rr))
...
class NumpyDataset(Dataset):
    def __init__(self, X: np.ndarray, y: np.ndarray, soft_logits: Optional[np.ndarray] = None):
        ...
        self.soft = None if soft_logits is None else torch.from_numpy(soft_logits).float()
...
def build_features_and_splits(cfg, device: torch.device):
    ...
    def prep(df):
        ...
        df[cfg.label_col] = df[cfg.label_col].apply(normalize_accent_3way)
    ...
    return feat, (y_priv, y_aux, y_dev, y_test)