...
def teacher_cache_id(cfg) -> str:
    if getattr(cfg, "precomputed_bundle", None):
        return _safe_tag(f"seed{cfg.seed}_sigma{cfg.sigma:g}_{cfg.teacher_backend}_"
                         f"{cfg.model_teacher}_dp{int(cfg.dp_teacher)}_C{cfg.max_grad_norm:g}_"
                         f"{cfg.loss_name}_prior{cfg.teacher_counts_source}_"
                         f"ep{cfg.epochs_teacher}_lr{cfg.lr_teacher:g}_bs{cfg.batch_size}")
...
def train_teacher(
    ...
    sample_rate = getattr(train_loader_use, "sample_rate", None)
    if sample_rate is None or float(sample_rate) <= 0.0:
        sample_rate = float(getattr(train_loader, "batch_size", 0) or 0) / max(
            1, len(getattr(train_loader, "dataset", []))
        )
    info = dict(
        dp=bool(dp),
        sigma=float(sigma),
        ...
        accountant=(dict(
            ...
            dataset_size=int(len(getattr(train_loader, "dataset", []))),
            nominal_batch_size=int(getattr(train_loader, "batch_size", 0) or 0),
            steps=int(epochs * len(train_loader_use)),
            accountant=privacy_engine.accountant.mechanism(),
            history=list(privacy_engine.accountant.history),
            sampling="poisson",
            adjacency="add_remove_one_training_record",
        ) if dp else None),
    )
    return model, info
